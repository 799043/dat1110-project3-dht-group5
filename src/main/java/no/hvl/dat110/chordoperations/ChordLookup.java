...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		BigInteger nodeIdPluss1 = this.node.getNodeID().add(BigInteger.ONE);
		...
		if (Util.checkInterval(key, nodeIdPluss1, succId)) {
			return successor;
		} else {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		int tableSize = fingerTable.size();
		for (NodeInterface n : fingerTable) {
			...
			if (Util.checkInterval(n.getNodeID(), this.node.getNodeID().add(BigInteger.ONE), ID.subtract(BigInteger.ONE))) {
				return n;
			}
		}
